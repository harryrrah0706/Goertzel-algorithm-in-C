...
//	sample = (int) 32768.0*sin(2.0*PI*freq1*TICK_PERIOD*tick) + 32768.0*sin(2.0*PI*freq2*TICK_PERIOD*tick);
	...
	sample = sample >> 13;	// scale down sample to prevent overflow.
}
...
void clk_SWI_GTZ_0697Hz(UArg arg0)
{
   	...
   	delay_2 = delay_1;
   	delay_1 = delay;
   	N++;	// incrementing N until 205

   	if (N == 206)
   	{
   		// calculating the square of Qn and Qn-1
   		prod1 = (delay_1 * delay_1);
   		prod2 = (delay_2 * delay_2);

   		// calculating goertzel value.
    	...
    	gtz_out[0] = Goertzel_Value;	//assigning value to array gtz_out at position 0 since this task only detects one frequency.
}
